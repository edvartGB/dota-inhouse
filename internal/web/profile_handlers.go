...
func (s *Server) handleProfileUpdate(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.store.UpdateUserDisplayName(r.Context(), user.SteamID, displayName); err != nil {
		log.Printf("Failed to update display name for %s: %v", user.SteamID, err)
		redirectProfileError(w, r, "Failed to update display name")
		return
	}
